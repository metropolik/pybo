...
void WorldSession::Update(void)
{
    ...
    {
        WorldPacket *pkt = sendPktQueue.front();
        sendPktQueue.pop();
        SendWorldPacket(*pkt);
        delete pkt;
    }
    ...
    while(pktQueue.size())
    {
        HandleWorldPacket(pktQueue.front());
        pktQueue.pop();
    }
...
void WorldSession::_HandleAuthChallengeOpcode(WorldPacket& recvPacket)
{
    ...
    logdebug("About to send WorldAuthChallenge");
    SendWorldPacket(auth);
    logdebug("Sent WorldAuthChallenge");
